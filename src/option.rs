//! Option0 - Educational reimplementation of Option<T>
...
impl<T> Option0<T> {
    ...
    /// Maps a [`Option0<T>`] to [`Option0<U>`] by applying a function.
    /// ```
    /// use rustlib::option::{Option0, Some, None};
    /// assert_eq!(Some(5).map(|x| x * 2), Some(10));
    /// assert_eq!(None.map(|x: i32| x * 2), None);
    /// ```
    pub fn map<U, F: FnOnce(T) -> U>(self, f: F) -> Option0<U> {
        match self {
            Some(x) => Option0::Some(f(x)),
            None => Option0::None,
        }
    }

    /// Applies a function that returns a [`Option0`].
    /// ```
    /// use rustlib::option::{Option0, Some, None};
    /// let sq = |x: i32| Some(x * x);
    /// assert_eq!(Some(2).and_then(sq), Some(4));
    /// ```
    pub fn and_then<U, F: FnOnce(T) -> Option0<U>>(self, f: F) -> Option0<U> {
        match self {
            Some(x) => f(x),
            None => Option0::None,
        }
    ...
    /// ```
    pub fn filter<P: FnOnce(&T) -> bool>(self, predicate: P) -> Option0<T> {
        match self {
    ...
    /// Converts from `&Option0<T>` to `Option0<&T>`.
    /// ```
    /// use rustlib::option::{Option0, Some, None};
    /// let x = Some(String::from("hello"));
    /// assert_eq!(x.as_ref().map(|s| s.len()), Some(5));
    /// ```
    pub fn as_ref(&self) -> Option0<&T> {
        match self {
            Some(x) => Option0::Some(x),
            None => Option0::None,
        }
    }
    ...
    /// // x is now None
    /// ```
    pub fn take(&mut self) -> Option0<T> {
    ...
    /// ```
    pub fn or(self, other: Option0<T>) -> Option0<T> {
        match self {
            Some(x) => Some(x),
            None => other,
        }
    ...
    /// ```
    pub fn or_else<F: FnOnce() -> Option0<T>>(self, f: F) -> Option0<T> {
        match self {
            Some(x) => Some(x),
            None => f(),
        }
...
impl<T, U> Option0<(T, U)> {
    ...
    /// ```
    pub fn unzip(self) -> (Option0<T>, Option0<U>) {
        match self {
...
impl<T> Option0<Option0<T>> {
    /// Converts from [`Option0<Option0<T>>`] to [`Option0<T>`].
    /// ```
    /// use rustlib::option::{Option0, Some, None};
    /// Some(Some(42)).flatten(); // Some(42)
    /// Some(None::<i32>).flatten(); // None
    /// ```
    pub fn flatten(self) -> Option0<T> {
        match self {
            Some(inner) => inner,
            None => None,
        }
...
/// ```
/// use rustlib::option::{Option0, Some, None, zip};
/// zip(Some(1), Some("hello")); // Some((1, "hello"))
/// zip(Some(1), None::<&str>); // None
/// ```
pub fn zip<T, U>(a: Option0<T>, b: Option0<U>) -> Option0<(T, U)> {
    match (a, b) {
...
mod tests {
    ...
    fn test_or() {
        ...
        let none1: Option0<i32> = None;
        let none2: Option0<i32> = None;
    ...
    fn test_zip() {
        ...
        let none_str: Option0<&str> = None;
        assert_eq!(zip(Some(1), none_str), None);
        let none_int: Option0<i32> = None;
        assert_eq!(zip(none_int, Some("hello")), None);
        let none1: Option0<i32> = None;
        let none2: Option0<&str> = None;
        assert_eq!(zip(none1, none2), None);
    }

    #[test]
    fn test_unzip() {
        let x: Option0<(i32, &str)> = Some((1, "hello"));
        assert_eq!(x.unzip(), (Some(1), Some("hello")));

        let y: Option0<(i32, &str)> = None;
    ...
    fn test_flatten() {
        ...
        let none_inner: Option0<i32> = None;
        assert_eq!(Some(none_inner).flatten(), None);
        let none_outer: Option0<Option0<i32>> = None;
